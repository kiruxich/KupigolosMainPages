...
export type ScenePose = {
  ...
  transfer: number;
  wearing: number; pointing: number; speaking: number; reading: number; cue: number; listening: number;
...
export const START: ScenePose = {
  ...
  frontHandX: 464, frontHandY: 568, backHandX: 484, backHandY: 574, transfer: 0, wearing: 0, pointing: 0, speaking: 0, reading: 0, cue: 0, listening: 0,
...
export const END: ScenePose = {
  ...
  frontHandX: 1189, frontHandY: 295, backHandX: 1040, backHandY: 539, wearing: 1, pointing: 1, cue: 1, transfer: 1, chairX: 0, time: 9,
};
...
export function makeTimeline(pose: ScenePose, cta: HTMLAnchorElement, draw: () => void, complete: () => void) {
  ...
  timeline.to(pose, { reading: 0, listening: 1, duration: .9, ease: "sine.inOut" }, 4.35);
  // Listen back for 1.4 seconds with the earcups held, then direct attention
  // to the CTA. The hidden rig is prepared before its full-length reveal.
  timeline.set(pose, {
  ...
  timeline.to(pose, {
    ...
    pointing: 1, cue: 1, listening: 0, duration: 1.35, ease: "sine.inOut",
  }, 6.65);
  timeline.to(cta, { scale: 1.035, duration: .3, yoyo: true, repeat: 1 }, 8.2);
  return timeline;
}
